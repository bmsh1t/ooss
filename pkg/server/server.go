...
func (s *Server) setupRoutes() {
	...
	api.Get("/campaigns/:id/profiles", handlers.ListCampaignProfiles(s.config))
	api.Put("/campaigns/:id/profiles/:name", handlers.SaveCampaignProfile(s.config))
	api.Delete("/campaigns/:id/profiles/:name", handlers.DeleteCampaignProfile(s.config))
	api.Get("/campaigns/:id/report", handlers.GetCampaignReport(s.config))
	api.Get("/campaigns/:id/export", handlers.ExportCampaignReport(s.config))
	...
	api.Get("/vulnerabilities/groups", handlers.ListVulnerabilityGroups(s.config))
	api.Get("/vulnerabilities/summary", handlers.GetVulnerabilitySummary(s.config))
	api.Post("/vulnerabilities/bulk", handlers.BulkVulnerabilityAction(s.config))
