...
**Responses API Mode / Native Responses Tools:**
```yaml
...
Notes:
- `extra_llm_parameters.api_mode: responses` forces the LLM step transport to use `/responses`
- `extra_llm_parameters.use_responses_api: true` can be used as a boolean alias
- if the configured provider `base_url` already ends with `/responses`, the step automatically uses Responses mode
- for native Responses built-in tools, put raw tool objects in `extra_llm_parameters.tools`
