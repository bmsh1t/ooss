...
- `python3` for the semantic-search helper scripts used by the AI workflow family

Optional vector-search dependencies:

- `JINA_API_KEY` or `OPENAI_API_KEY` to enable embeddings-backed retrieval
- `chromadb` when running the Chroma hybrid-search fragment directly
