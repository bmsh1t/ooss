...
- optional `rerank_config` pointing at an OpenAI-compatible `/rerank` endpoint
...
When `rerank_config.enabled=true`, both `kb vector search` and `POST /knowledge/vector/search` can apply rerank and return `ranking_source` / `rerank_applied` metadata for downstream workflows.
