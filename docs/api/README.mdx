...
| [Campaigns](campaigns.mdx) | Group queued runs, rerun failed targets, and queue deep scans |
...
`superdomain-extensive`, `superdomain-extensive-stable`, `superdomain-extensive-hybrid`, and `superdomain-extensive-lite` consume the same knowledge workspace during semantic search, then persist normalized `applied-ai-decision` and `followup-decision` artifacts for downstream retest, operator, and campaign handling.
