...
`hello` just ran `trellis init` on a fresh clone, saw "Developer
...
- `.trellis/workspace/hello/journal-*.md` — their session log,
...
- Check if `.trellis/workspace/hello/` already exists — if yes, it's
  their journal from another machine and worth mentioning.
- Run `python3 ./.trellis/scripts/task.py list --assignee hello` to
...
python3 ./.trellis/scripts/task.py archive 00-join-hello
```
